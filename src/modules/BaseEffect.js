...
export class BaseEffect {
    constructor(audioContext, x, y, type, width, height, id, initialState) {
    ...
    async init() {
        try {
            ...
            // --- CORRECCIÓN --- Estandarizar nombres de puertos a 'In' y 'Out'
            this.inputs = { 'In': { x: 0, y: this.height / 2, type: 'audio', target: this.inputGain } };
            this.outputs = { 'Out': { x: this.width, y: this.height / 2, type: 'audio', source: this.outputGain } };
    ...
    drawKnob(ctx, paramName, label, x, y, min, max, value, isLogarithmic) {
        ...
        if (isLogarithmic) {
            ...
            const logValue = Math.log(Math.max(min || 0.001, value));
    ...
    drawConnectors(ctx, hovered) {
        ...
        Object.entries(this.inputs).forEach(([_name, props]) => { 
            ...
            ctx.fill();
            ctx.textAlign = 'left';
            ctx.fillText(_name.toUpperCase(), ix + connectorRadius + 4, iy + 4);
        });
    ...
    handleDragInteraction(worldPos) {
        ...
        let newValue;
        if (def.logarithmic) {
            const logMin = Math.log(def.min || 0.001);
            const logMax = Math.log(def.max);
            const logRange = logMax - logMin;
            const startValue = Math.max(def.min || 0.001, this.dragStart.value);
